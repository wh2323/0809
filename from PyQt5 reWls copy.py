...
class MyDialog(QDialog):
    ...
    def addStockToWatchlistFromCell(self, row, column):
        stock_name = self.tableWidget_2.item(row, 0).text()  # 주식 이름 (0번째 열)
         # 모든 행을 순회하여 주식 이름이 일치하는 행의 체크박스를 체크 상태로 설정
        for row_index in range(self.tableWidget.rowCount()):
            item = self.tableWidget.item(row_index, 0)  # 주식 이름이 있는 0번째 열의 아이템을 가져옴
            if item is not None and item.text() == stock_name:
                widget = self.tableWidget.cellWidget(row_index, 5)  # 6번째 열의 체크박스를 가져옴
                if widget is not None:
                    checkbox = widget.findChild(QCheckBox)
                    if checkbox is not None and not checkbox.isChecked():
                        checkbox.setChecked(True)  # 체크박스를 체크 상태로 설정
                        CheckBoxBoolean[row_index] = True  # 내부 상태도 업데이트
                        print(f"{row_index} : {CheckBoxBoolean[row_index]}")
                break  # 주식 이름이 일치하는 행을 찾으면 반복 종료
    ...
    def removeFromWatchlist(self, stock_name=None):
        ...
        if stock_name:
            # 사용자에게 정말 제거할 것인지 물어보는 메시지 박스 생성
            reply = QMessageBox.question(
                self,
                "확인",
                f"{stock_name}을(를) 관심 종목 목록에서 제거하시겠습니까?",
            ...
            if reply == QMessageBox.Yes:
                ...
                print(f"{stock_name} 제거가 취소되었습니다.")  # 취소 시 콘솔에 출력 (필요 시 수정 가능)
    
                # No를 눌렀을 때 체크박스를 다시 체크 상태로 설정
                for row_index in range(self.tableWidget.rowCount()):
                    item = self.tableWidget.item(row_index, 0)  # 주식 이름이 있는 0번째 열의 아이템을 가져옴
                    if item is not None and item.text() == stock_name:
                        widget = self.tableWidget.cellWidget(row_index, 5)  # 6번째 열의 체크박스를 가져옴
                        if widget is not None:
                            checkbox = widget.findChild(QCheckBox)
                            if checkbox is not None and not checkbox.isChecked():
                                checkbox.setChecked(True)  # 체크박스를 다시 체크 상태로 설정
                                CheckBoxBoolean[row_index] = True  # 내부 상태도 업데이트
                                print(f"{row_index} : {CheckBoxBoolean[row_index]} (체크박스가 다시 체크됨)")
                        break  # 주식 이름이 일치하는 행을 찾으면 반복 종료
